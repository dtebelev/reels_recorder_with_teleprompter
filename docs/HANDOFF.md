...
**Not yet re-confirmed by the user** as of this writing — check the Review screen again on the next round of testing.
...
1. **Confirm the button-clipping fix and the native-video-chrome fix (Eighth Round) on-device.** Both are deployed but not yet explicitly confirmed by the user.
