...
**The active-quality-button highlight had the project's recurring CSS bug for a fourth time.** `.row__btn--active { background: #ff5a36; ... }` (a lone class, specificity 0,1,0) lost to `.row button { background: #1a1a1c; ... }` (class+element, 0,1,1) — same trap as `.record-btn`, `.pause-btn`/`.stop-circle-btn`, all fixed earlier in this file by scoping the override to `.container .modifier-class` instead of `.modifier-class` alone. Fixed the same way (`.row .row__btn--active`). **If a new "state" class added to a button inside `.row` or `.controls` doesn't seem to visually apply, check this first before anything else** — it has now happened four times and is always the same cause. **Confirmed fixed by the user** ("Работает").

## Eighth round: native video-player chrome collided with our own UI (2026-09-18)

Reported with a screenshot of the Review screen: three icons crowded together near the top edge — an AirPlay/casting icon (confirmed by the user, "динамик с правой стороны"), a fullscreen-toggle icon, and our own diagnostics ⓘ button, all bunched under the notch. These aren't all our elements: `<video controls>` draws its own native chrome (AirPlay/cast top-right, fullscreen-toggle top-left, plus a "..." menu and scrub bar along the bottom) *inside the video element's own box*, and with `#review-video` filling the entire screen, that native chrome rendered at the physical screen edges — the same territory as our diagnostics button above and the Keep/Retake bar below.

**Fix, two parts** (`js/app.js`'s `#review-video` markup and its CSS in `css/styles.css`):
1. Disabled the native chrome not needed here: `disablePictureInPicture`, `disableRemotePlayback` (hides AirPlay), the legacy `x-webkit-airplay="deny"` (older WebKit), and `controlsList="nofullscreen noremoteplayback noplaybackrate"`.
2. Inset `#review-video` from the top/bottom edges (`position: absolute` with `top`/`bottom` insets, safe-area-aware) instead of filling the screen, so whatever native controls remain (play/pause, the 10s skip buttons, the scrub bar) draw within their own box, clear of the diagnostics button and the Keep/Retake bar.

**Update: the video-inset fix didn't work.** The user re-tested and the play/skip-10s buttons still weren't visually centered — the "make top/bottom insets equal" theory either doesn't apply the way expected, or something else about how Safari centers native controls within a `contain`-fit video box isn't what was assumed. Rather than try a third inset-tuning guess, **native `<video controls>` was dropped entirely** and replaced with custom controls (`setupCustomVideoControls()` in `js/app.js`): a single centered play/pause button (`.play-toggle`, fades out while playing, tap the video or the button to toggle) and a thin custom progress bar (`.progress-bar`/`.progress-bar__fill`, visual only — not currently seekable/draggable). This sidesteps the centering problem entirely by not depending on native control positioning, and also permanently removes the AirPlay/cast/fullscreen icon clutter from Round Eight since there's no native chrome left to draw them. The video element itself went back to filling the full screen (`inset: 0`) since there's no more native-chrome collision to avoid.

**Trade-off, resolved:** the user asked for custom seeking back, so the progress bar is now interactive — drag or tap it to scrub (`pointerdown`/`pointermove` on `#progress-bar`, a `dragging` flag suppresses the `timeupdate`-driven paint while the finger is down so the thumb tracks the touch instead of the still-playing video). The visible line is 3px, but the touch target wrapping it is a full 32px tall (`.progress-bar`, `touch-action: none` so the browser doesn't try to scroll/interpret it as a page gesture) — real controls need more finger-room than the line looks like it needs. Still no 10-second skip buttons; add them only if actually requested, they weren't part of this ask.

## Ninth round: pill-style speed buttons, app-wide first-tap fix, video-control rework (2026-09-18)

Three more fixes, the middle one broader than the others:

**"Медленнее"/"Быстрее" restyled as pill capsules.** The user sent two reference screenshots (a warm-toned dark capsule button, serif small-caps label, thin border — shown alongside a circular stop button in the second image) and asked to match that look. Gave both buttons a shared `.speed-btn` class: translucent dark capsule (`border-radius: 999px`), thin warm-white border, `Georgia`/serif, uppercase, letter-spaced. Scoped as `.controls .speed-btn` (two classes) rather than `.speed-btn` alone — see the repeated CSS-specificity note elsewhere in this file; a fifth recurrence of that exact bug was avoided by scoping it correctly the first time here.

**App-wide "doesn't register on the first tap" — CONFIRMED FIXED.** Reported as happening across the whole app, not one screen. Cause: every screen here is a freshly-built `<button>` via `innerHTML` on each transition, and iOS Safari has a known quirk where a dynamically-inserted interactive element with no explicit `cursor` can eat a first tap before clicks register normally. Fixed with one global rule: `button, a, [role="button"] { cursor: pointer; touch-action: manipulation; }`. **User confirmed**: "Кнопки начали работать нормально, спасибо."

**Review screen's native video controls replaced with custom ones.** Two rounds of trying to get the native `<video controls>` play/skip-10s buttons to visually center (asymmetric insets, then symmetric insets) both failed per follow-up screenshots — the assumption that adjusting the video element's box insets controls where Safari centers its native control icons turned out not to hold, at least not in a way two attempts correctly predicted. Rather than guess a third inset value, dropped `<video controls>` and `controlsList`/`disableRemotePlayback` etc. entirely, replacing it with fully custom controls in `setupCustomVideoControls()` (`js/app.js`): a `.play-toggle` button placed with `position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%)` (exact, guaranteed centering — no more native-positioning guessing), and a custom `.progress-bar` with a draggable thumb (`pointerdown`/`pointermove`, seeks `video.currentTime`, suppresses the `timeupdate`-driven repaint while dragging via a `dragging` flag so the thumb doesn't snap back to playback position mid-drag). This also permanently resolved the Eighth Round's AirPlay/fullscreen-icon clutter, since there's no native chrome left to draw those icons at all. **Not yet confirmed by the user as of this writing** — the centering fix and the scrub-by-dragging addition were both requested but the result hasn't been checked back yet.

**Lesson reinforced:** this is the *second* time in this project that guessing at CSS positioning to control native browser-rendered UI (safe-area insets for video controls, similar in spirit to the earlier localtunnel/deploy-workflow guessing) failed twice before switching to "build it ourselves instead of fighting the browser." When two attempts to nudge a native/OS-controlled element into place both fail, stop adjusting numbers and replace the native thing with something fully custom-positioned instead.

## Status as of 2026-09-18 (end of Ninth Round): stable, nearly everything confirmed fixed
