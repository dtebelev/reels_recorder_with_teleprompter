...
5. **Recordings sometimes "got stuck" / never finished.** Reported directly ("несколько видео... не делаются до конца") after making several recordings. Root cause not confirmed (couldn't reproduce without a device), but two real risk factors were identified and addressed defensively:
   ...
   - Added a hard safety net regardless of root cause: `recorder.stop()` is now wrapped in an 8-second timeout (`withTimeout()` in `js/app.js`). If it doesn't resolve in time, the user is shown an error screen with a way back to Setup instead of being stuck on a dead Recording screen forever. **This is a symptom fix, not a confirmed root-cause fix** — if recordings keep getting stuck (even if now recoverable via the timeout), the underlying cause needs more investigation, likely by checking Safari's remote Web Inspector console during an actual hang (connect the iPhone to a Mac via cable, Settings > Safari > Advanced > Web Inspector, then Safari on Mac > Develop menu) since none of this could be diagnosed from console output in this session.
