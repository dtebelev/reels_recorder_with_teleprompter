...
**Trade-off:** lost the native 10-second skip-forward/back buttons and native scrubbing-by-dragging (the progress bar is currently display-only). If the user wants to seek within a take before deciding Keep/Retake, that's now only possible by pausing/playing, not dragging — revisit if that's missed. Not yet confirmed working by the user.
